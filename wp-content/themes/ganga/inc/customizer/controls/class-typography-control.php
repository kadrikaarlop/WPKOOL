...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Ganga_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Ganga_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'ganga-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'ganga-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), GANGA_VERSION, true );
			wp_enqueue_style( 'ganga-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), GANGA_VERSION );

			wp_enqueue_script( 'ganga-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'ganga-typography-selectWoo' ), GANGA_VERSION, true );
			wp_enqueue_style( 'ganga-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), GANGA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'ganga_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'ganga' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'ganga' );
			$this->json[ 'google_fonts' ] = apply_filters( 'ganga_typography_customize_list', ganga_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = ganga_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'ganga' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'ganga' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'ganga' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'ganga' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="ganga-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="ganga-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="ganga-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="ganga-font-transform">
					<label>
