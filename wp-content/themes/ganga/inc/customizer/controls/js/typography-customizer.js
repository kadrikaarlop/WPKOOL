( function( api ) {
	...
	api.controlConstructor['ganga-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.ganga-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the ganga_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'ganga_get_all_google_fonts_ajax',
								ganga_customize_nonce: ganga_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.ganga-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.ganga-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.ganga-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.ganga-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.ganga-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.ganga-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.ganga-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.ganga-font-variant' );
			...
			control.container.on( 'change', '.ganga-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.ganga-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.ganga-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.ganga-font-family select' ).selectWoo();
	jQuery( '.ganga-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".ganga-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.ganga-font-variant' ).hide();
		}
	});
