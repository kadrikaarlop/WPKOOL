...
if ( ! function_exists( 'ganga_get_setting' ) ) {
	/**
	 * A wrapper function to get our settings.
	 *
	 */
	function ganga_get_setting( $setting ) {
		$ganga_settings = wp_parse_args(
			get_option( 'ganga_settings', array() ),
			ganga_get_defaults()
		);

		return $ganga_settings[ $setting ];
	}
}

if ( ! function_exists( 'ganga_get_layout' ) ) {
	/**
	...
	function ganga_get_layout() {
		...
		$layout = $ganga_settings['layout_setting'];

		// Get the individual page/post sidebar metabox value
		$layout_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_ganga-sidebar-layout-meta', true ) : '';
		...
		if ( is_single() && ! $buddypress ) {
			$layout = null;
			$layout = $ganga_settings['single_layout_setting'];
		}
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = null;
			$layout = $ganga_settings['blog_layout_setting'];
		}

		// Finally, return the layout
		return apply_filters( 'ganga_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'ganga_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function ganga_get_footer_widgets() {
		// Get current post
		global $post;
		...
		$widgets = $ganga_settings['footer_widget_setting'];

		// Get the individual footer widget metabox value
		$widgets_meta = ( isset( $post ) ) ? get_post_meta( $post->ID, '_ganga-footer-widget-meta', true ) : '';
		...
		// Finally, return the layout
		return apply_filters( 'ganga_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'ganga_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 *
	 */
	function ganga_show_excerpt() {
		// Get current post
		global $post;
		...
		$more_tag = apply_filters( 'ganga_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		// Check the post format
		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		// Get the excerpt setting from the Customizer
		$show_excerpt = ( 'excerpt' == $ganga_settings['post_content'] ) ? true : false;
		...
		// Return our value
		return apply_filters( 'ganga_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'ganga_show_title' ) ) {
	/**
	 ...
	 */
	function ganga_show_title() {
		return apply_filters( 'ganga_show_title', true );
	}
}

if ( ! function_exists( 'ganga_padding_css' ) ) {
	/**
	 ...
	 */
	function ganga_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'ganga_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'ganga_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function ganga_get_navigation_location() {
		return apply_filters( 'ganga_navigation_location', ganga_get_setting( 'nav_position_setting' ) );
	}
}
