...
add_action( 'after_setup_theme', 'ganga_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function ganga_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'ganga_construct_sidebars' );
}

if ( ! function_exists( 'ganga_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'ganga_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function ganga_woocommerce_start() { ?>
...
if ( ! function_exists( 'ganga_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'ganga_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function ganga_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'ganga_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ganga_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function ganga_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'ganga_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'ganga_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ganga_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function ganga_bbpress_css() {
...
if ( ! function_exists( 'ganga_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ganga_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function ganga_buddypress_css() {
...
if ( ! function_exists( 'ganga_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ganga_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function ganga_beaver_builder_css() {
